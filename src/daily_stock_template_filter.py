...
class DailyStockTemplateFilter:
    ...
    def add_basic_stock_info(self):
        ...
        self.allstock['year high sort'] = ((self.allstock['250Max'] - self.allstock['Adj Close']).abs() / self.allstock['250Max']) < 0.19
    ...
    def calculate_rs_diff_strategies(self):
        ...
        if self.yesterday_allstock is not None:
            ...
            for key, value in rs_diff_conditions.items():
                value = value.dropna()
                value = value.drop(value.index[value.index.duplicated()])
                self.allstock.loc[:, key] = value
            diff_conditions = {
            ...
            for key, condition in diff_conditions.items():
                value = condition.dropna()
                self.allstock[key] = condition
        else:
            self.allstock['RS EMA20 20MAX diff'] = 0
            self.allstock['RS EMA20 diff'] = 0
            diff_conditions = ['RS EMA20diff < -5', 'RS EMA20diff < -8', 'RS EMA20diff < -11', 'RS EMA20 20MAX diff < -5', 'RS EMA20 20MAX diff < -10', 'RS EMA20 20MAX diff < -20']
            for key in diff_conditions:
                self.allstock[key] = False
    ...
    def apply_templates(self):
        ...
        """
        self.allstock['T5'] = self.allstock[['RS 20rate>80', 'RS 250rate>55', 'RS 250rate<75', 'year low sort', 'Volume 50MA>150k']].all(axis=1)
        self.allstock['T5-2'] = self.allstock[['RS EMA20rate>80', 'RS EMA250rate>60', 'RS EMA250rate<80', 'year low sort', 'Volume 50MA>150k']].all(axis=1)
        self.allstock['T6'] = self.allstock[['RS EMA250rate>85', 'RS 250EMA 50MIN > 50', 'RS EMA50rate>75', 'Volume 50MA>250k', 'year low sort', 'year high sort']].all(axis=1)# , 'RS EMA50rate<95', 'RS 50EMA 20MIN >30'
        self.allstock['T11'] = self.allstock[['RS EMA250rate>75', 'RS 250EMA 50MIN > 30', 'RS 20EMA is 10MAX', 'Volume 50MA>250k', 'price>95%50MA', 'year low sort', 'year high sort']].all(axis=1) # , 'RS 50EMA 20MIN >65'
    ...
    def save_to_excel(self, output_directory: str, today: datetime, mark: str = ''):
        """
        Save the filtered stock data to a CSV file.
        """
        output_directory = os.path.join(output_directory, f"daily_stock_summary_{today}_with_template{mark}.xlsx")
        self.allstock.to_excel(output_directory, index=True)

    def run(self, output_directory: str, today: datetime, mark: str = '') -> pd.DataFrame:
        """
        ...
        self.save_to_excel(output_directory, today, mark)
